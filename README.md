...
| (주문) 완료 (상태) | Completed | 주문 과정이 종결된 상태로 추가적인 작업이 필요하지 않음. 배달 주문인 경우 배달이 완료된 경우, 그렇지 않은 경우 서빙이 완료된 상태여야 주문이 완료될 수 있다. |
| 서빙 | Serving | 손님이 주문한 메뉴를 손님에게 직접 또는 배달 대행사의 배달원에게 전달하는 행위. |
| 서빙 완료 (상태) | Served | 서빙을 통해 메뉴를 손님에게 직접 또는 배달 대행사의 배달원에게 전달 완료한 상태. |
