...
- `사장님`은 `상품 가격`을 변경할 수 있다.
  - `상품 가격`이 변경되면 해당 상품이 포함된 메뉴의 가격이 메뉴 상품들의 가격보다 싸면 메뉴가 `숨김 처리`된다.
  - ![img_1.png](img_1.png)
...
- `사장님`은 `메뉴`를 등록할 수 있다.
  ...
  - `메뉴 가격`은 0원 이상이어야 한다.
  - `메뉴 가격`이 `메뉴`에 속한 상품 금액의 합보다 클 수 없다.
  - `메뉴 그룹`을 설정할 수 있다.
  - `메뉴 노출 상태`를 설정할 수 있다.
...
- `사장님`은 `메뉴 노출 상태`를 수정할 수 있다.
  - `노출 메뉴`로 변경 시 `메뉴 가격`이 `메뉴`에 속한 상품 금액의 합보다 클 수 없다.
  - `숨김 메뉴`로 변경할 수 있다.
  - ![img_2.png](img_2.png)
...
### 배달 주문
- `사장님`은 `배달 주문`을 등록할 수 있다.
  - 하나 이상의 `메뉴`를 등록해야 한다.
  - `메뉴`의 수량은 0개 이상이어야 한다.
  - `노출 메뉴`만 등록할 수 있다.
...
// TODO domo - 주문 상태 시퀀스 다이어그램 추가, 그림 용어 통일, 그림과 글의 문장 통일