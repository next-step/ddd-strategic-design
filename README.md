...
| 메뉴 그룹을 등록하다       | Add a menu group           | 새로운 메뉴 그룹을 추가하는 것을 의미                                                                                |
| 메뉴 그룹 목록을 조회하다    | List menu groups           | 등록된 모든 메뉴 그룹을 보는 것을 의미                                                                               |
|                   |                            |                                                                                                      |
| 메뉴                | Menu                       | 판매하기 위한 상품과 개수, 가격을 미리 정하여 묶어놓은 주문 단위. 실질적으로 주문은 메뉴를 대상으로 이루어진다.                                     |
| 메뉴를 (메뉴 그룹에) 등록하다 | Add a menu (to menu group) | 메뉴를 (메뉴 그룹에) 추가하는 행위를 말한다.                                                                           |
| 메뉴 이름             | Menu name                  | 메뉴를 추가할 때 정한 메뉴의 이름. 상품의 이름과 별도로 필요하다.                                                               |
| 메뉴에 속한 상품         | Menu product               | 메뉴를 추가할 때 메뉴에 포함된 상품                                                                                 |
| 메뉴에 속한 상품 수량      | Menu product quantity      | 메뉴가 주문될 때 메뉴에 속한 상품을 몇 개 팔지, 메뉴를 등록할 때 정한다.                                                          |
| 메뉴 가격             | Menu price                 | 메뉴를 등록할 때 지정한 가격. 주문할 때의 지불 기준은 상품 가격이 아니라 이것이다                                                      |
| 메뉴 가격을 변경하다       | Change menu price          | 변경한 이후로는 메뉴를 주문할 때 변경된 가격을 기준으로 주문된다.                                                                |
| 노출된 메뉴            | Displayed menu             | 메뉴는 노출되어야 주문할 수 있다.                                                                                  |
| 메뉴를 노출하다          | Display a menu             | 메뉴를 노출하여 주문할 수 있게 만들다.                                                                               |
| 숨겨진 메뉴            | Hided menu                 | 숨겨진 메뉴는 주문할 수 없다.                                                                                    |
| 메뉴를 숨기다           | Hide a menu                | 메뉴를 숨겨 주문할 수 없게 만들다.                                                                                 |
| 메뉴 목록을 조회하다       | List menus                 | 등록된 모든 메뉴를 보는 것을 의미                                                                                  |
|                   |                            |                                                                                                      |
| 주문 테이블(= 테이블)     | Order table(a.k.a. Table)  | 매장 내에서 식사를 할 때 차지하는 공간. (이름, 손님 수, 비어 있는 지 여부)                                                       |
| 주문 테이블명           | Order table name           | 테이블을 구분할 수 있는 이름                                                                                     |
...
| 손님 수를 변경하다        | Change number of guests    | 차지된 테이블의 손님 수를 바꾼다.                                                                                  |
| 테이블 목록을 조회하다      | List order tables          | 모든 테이블 목록을 조회하는 것을 의미                                                                                |
|                   |                            |                                                                                                      |
