...
| 금지어 | banned word| **상품**의 이름으로 사용하기 적합하지 않은 말       |
...
| 금지어   | banned word    | **메뉴**의 이름으로 사용하기 적합하지 않은 말                                  |
...
## 메뉴
- `메뉴 (Menu)` 는 하나 이상의 `메뉴 상품 (Menu Product)` , 이름, 가격, 노출 여부, `메뉴그룹(Menu Group)`을 가진다.
  - 가격은 0보다 커야 하며, 속한 `메뉴 상품 (Menu Product)`의 가격의 합보다 작거나 같은 가격을 가져야 한다.
...
sequenceDiagram
    ...
    autonumber
		...
		Admin ->> System : 주문 완료
```
- `매장 주문(Eat In order)` 주문은 `주문테이블(order table)`을 가진다.
- `매장 주문(Eat In order)`은  **요청 →** **수락** → **준비 완료** → **주문 완료** 순서로 진행된다.
  - 주문을 요청한다.
    - 주문을 요청하면 주문 대기 중 상태가 된다.
  - 주문을 수락한다.
    - 대기 중 상태의 주문만 수락이 가능하다.
    - 주문을 수락하면 수락 상태가 된다.
  - 주문을 준비완료 한다.
    - 수락 상태의 주문만 준비완료가 가능하다. 
    - 주문을 준비완료 하면 준비완료 상태가 된다.
  - 주문을 주문완료 한다.
    - 준비 완료 상태의 주문만 주문완료가 가능하다.
    - 주문 완료하면 주문 완료 상태가 된다.
- 주문이 완료된 `매장 주문(Eat In order)`은 테이블을 `정리(clean)` 한다.
...
- `배달주문(delivery order)` 은 **요청** → **수락** → **준비완료** → **배달시작** → **배달완료** → **주문완료** 순서로 진행된다.
  ...
  - 주문을 준비완료 한다.
    ...
    - 주문을 준비완료 하면 준비완료 상태가 된다.
  - 주문을 배달시작 한다.
    - 준비 완료 상태의 주문만 배달시작이 가능하다. 
    - 주문을 배달시작하면 배달시작 상태가 된다.
  - 주문을 배달완료 한다.
    - 배달 시작 상태의 주문만 배달 완료가 가능하다.
    - 주문을 배달완료하면 배달완료 상태가 된다.
  - 주문을 주문완료 한다.
    - 배달 완료 상태의 주문만 주문완료가 가능하다.
    - 주문 완료하면 주문 완료 상태가 된다. 
