...
### Product
#### 속성
- **상품 이름 (Product Name)**: 0자 이상의 비속어가 포함되지 않은 문자열
- **상품 가격 (Product Price)**: 0 이상의 숫자

#### 행동
- **상품 등록 (Create Product)**
  - 상품 이름은 0자 이상이어야 하며, 비속어를 포함하지 않아야 한다.
  - 상품 가격은 0 이상이어야 한다.
- **상품 가격 변경 (Change Product Price)**
  - 변경된 가격으로 인해 해당 상품을 포함하는 '메뉴 구성 상품 가격의 총합'이 기존 메뉴 가격보다 낮아지면 해당 메뉴를 비공개한다.

### Menu
#### 속성
- **메뉴 그룹 (Menu Group)**
- **메뉴 이름 (Menu Name)**: 0자 이상의 비속어가 포함되지 않은 문자열
...
- **메뉴 등록 (Create Menu)**
  - 메뉴 그룹이 있어야 한다.
  - 메뉴 구성 상품이 1개 이상이어야 한다.
  - 메뉴 이름은 0자 이상이어야 하며, 비속어를 포함하지 않아야 한다.
  - 메뉴 가격은 0 이상이어야 하며,'메뉴 구성 상품 가격의 총합'보다 같거나 작아야 한다.
- **메뉴 가격 변경 (Change Menu Price)**
  - 변경할 가격은 '메뉴 구성 상품 가격의 총합'보다 같거나 작아야 한다.
- **메뉴 공개 (Display Menu)**
  - 메뉴 가격이 '메뉴 구성 상품 가격의 총합'보다 같거나 작아야 한다.
- **메뉴 비공개 (Hide Menu)**
