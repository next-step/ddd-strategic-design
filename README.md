...
| 한글명       | 영문명                | 설명                                             |
|-----------|--------------------|------------------------------------------------|
| 메뉴        | Menu               | 고객에게 판매를 목적으로 보유한 물건으로 메뉴를 구성하는 단위.            |
| 메뉴 이름     | Menu Name          | 메뉴의 이름, 비속어를 사용할 수 없다.                         |
| 비속어  | profanity  | 욕설 또는 저속한 표현. |
| 메뉴 가격     | Menu Price         | 메뉴의 가격, 0원 이상이여야 하고 메뉴 속한 상품 * 수량의 합보다 클 수 없다. |
| 메뉴 상품     | Menu Product       | 메뉴에 포함되고 수량이 있는 상품으로 구성된다.                     |
...
| 주문 테이블 이름  | Order Table Name | 테이블의 이름을 나타낸다.                |
...
| 주문 상태(준비 완료)  | SERVED                                              | 배달 주문이 준비 완료 되어 배달대행사에 배달 요청한 상태 |
