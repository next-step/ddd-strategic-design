...
| 한글명 | 영문명 | 설명 |
| --- | --- | --- |
| 비속어 | profanity | 욕설 또는 비속어로 손님들이 불편함을 느껴 메뉴명 또는 상품명에 포함될 수 없는 단어를 의미한다. |
...
| 매장 주문 | EAT_IN | 매장에서 식사하기 위해 주문하는 경우다. |
| 포장 주문 | TAKEOUT | 포장 주문하는 경우다. |
| 배달 주문 | DELIVERY | 배달 주문하는 경우다. |
...
- `주문 유형(orderType)`이 `매장 식사(EAT_IN)` 이다.
...
- `주문 유형(orderType)`이 `배달 주문(DELIVERY)` 이다.
- `주문(order)`은 `대기(waiting)` → `수락(accepted)` → `서빙 완료(served)` → `배달 중(delivering)` → `배달 완료(delivered)` → `주문 처리 완료(completed)` 순서로 진행된다.
<br/>
<br/>
- `배달 주문(DELIVERY)`를 생성한다.
  - 존재하지 않는 `메뉴(menu)`나 `숨겨진 메뉴(hiddenMenu)`는 주문할 수 없다.
  - `주문 상품(orderLineItem)`이 하나 이상이어야 한다.
  - 각 `주문 상품(orderLineItem)`의 개수는 0개 이상이어야 한다.
  - `주문상품(orderLineItem)`의 가격은 실제 `메뉴 가격(menuPrice)`과 일치해야 한다.
  - `배달 주소(deliveryAddress)`가 있어야 한다.
- `배달 주문(DELIVERY)`를 `수락한다(accept).`
  - `대기(waiting)` 상태의 `주문(order)`만 `수락할 수 있다(accept).`
  - `배달 대행사(deliveryAgency)`에 배달을 요청한다.
- `배달 주문(DELIVERY)`를 `서빙한다(serve).`
  - `수락(accped)` 상태의 `주문(order)`만 `서빙할 수 있다(serve).`
- `배달 주문(DELIVERY)`를 `배달을 시작한다(startDelivery).`
  - `서빙완료(served)` 된 `주문(order)`만 `배달을 시작할 수 있다(startDelivery).`
- `배달 주문(DELIVERY)`를 `배달을 완료한다(completeDelivery).`
  - `배달 중(delivering)`인 `주문(order)`만 `배달을 완료할 수 있다(completeDelivery).`
- `배달 주문(DELIVERY)`를 `완료한다(complete).`
  - `배달 완료(delivered)`된 `주문(order)`만 `완료할 수 있다(complete).`

### 포장 주문

- `포장 주문(TAKEOUT)`은 식별자, 주문 상태, 주문 시간, 주문 상품 목록을 가진다.
- `주문 유형(orderType)`이 `포장 주문(TAKEOUT)` 이다.
- `포장 주문(TAKEOUT)`은 `대기(waiting)` → `수락(accepted)` → `서빙 완료(served)` → `주문 처리 완료(completed)` 순서로 진행된다.
<br/>
<br/>
- `포장 주문(TAKEOUT)`을 생성한다.
  - 존재하지 않는 `메뉴(menu)`나 `숨겨진 메뉴(hiddenMenu)`는 주문할 수 없다.
  - `주문 상품(orderLineItem)`이 하나 이상이어야 한다.
  - 각 `주문 상품(orderLineItem)`의 개수는 0개 이상이어야 한다.
  - `주문상품(orderLineItem)`의 가격은 실제 `메뉴 가격(menuPrice)`과 일치해야 한다.
- `포장 주문(TAKEOUT)`을 `수락한다(accept).`
  - `대기(waiting)` 상태의 `주문(order)`만 `수락할 수 있다(accept).`
- `포장 주문(TAKEOUT)`을 `서빙한다(serve).`
  - `수락(accped)` 상태의 `주문(order)`만 `서빙할 수 있다(serve).`
