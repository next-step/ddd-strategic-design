...
- 상품(Product)을 등록할 수 있다
  - 상품의 이름에는 비속어(Banned Word)가 포함될 수 없다.
- 상품 가격을 변경할 수 있다
  - 상품의 가격은 0원 이상만 가능하다.
...
- 주문 테이블(Order Table)은 이름을 가진다.
- 주문 테이블은 방문한 손님(Guest) 수를 가진다.
...
- 빈 테이블(Empty Table)로 설정할 수 있다.
- 사용 중인 테이블(Using table)로 설정할 수 있다.
- 손님 수를 변경할 수 있다
...
- 주문(Order)은 주문 종류(Order Type, 포장, 매장, 배달)를 가진다.
  - 배달 주문은 배달 주소를 가진다.
  - 매장 주문은 주문 테이블을 가진다.
- 주문은 하나 이상의 주문 항목(Order Item)을 가진다.
- 주문 상태(Order Status)는 접수 대기, 접수, 서빙, 배달 중, 배달완료, 주문완료가 있다.
...
- 주문을 서빙할 수 있다.
  - 상태가 접수인 주문만 서빙(Served)할 수 있다.
- 배달 주문을 배달(Delivery)할 수 있다.
  - 상태가 서빙(Served)인 주문만 배달할 수 있다.
- 배달 주문을 배달 완료(Delivered)로 설정할 수 있다.
- 주문을 완료(Done)로 설정할 수 있다.
