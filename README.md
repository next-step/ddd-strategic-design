...
| 메뉴 > `노출/숨김 여부` | Displayed    | 메뉴를 고객에게 보여줄지의 여부를 뜻한다.                                      |
...
| 주문 > 주문 상태 > `접수 대기` | Waiting          | 고객이 주문한 주문한 메뉴(들)이 매장에서는 접수하기 전 상태를 뜻한다.                           |
| 주문 > 주문 상태 > `접수됨`   | Accepted         | 주문을 확인하였고, 정상적으로 진행하겠다는 상태를 뜻한다.                                   |
| 주문 > 주문 상태 > `서빙됨`   | Served           | 매장 주문, 포장 주문에서 고객에게 혹은 배달 주문에서 배달 기사에게 주문한 메뉴(들)이 전달된 상태를 뜻한다.     |
...
| 주문 > 주문 상태 > `주문 완료` | Completed        | 주문의 처리를 완료했다는 상태를 뜻한다.                                             |
