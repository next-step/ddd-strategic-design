...
- `배달 주문`은 현재 `배달 주문 순서`를 알 수 있는 `배달 주문 상태`를 가진다.
- `배달 주문 상태`는 이전 `배달 주문 상태`와 `배달 주문 순서`를 고려해 `배달 주문 상태`를 바꾼다.

### 6. 포장 주문

- `포장 주문`은 `손님`에게 조리된 음식을 포장 해준다.
- `포장 주문`은 현재 `포장 주문 순서`를 알 수 있는 `포장 주문 상태`를 가진다.
- `포장 주문 상태`는 이전 `포장 주문 상태`와 `포장 주문 순서`를 고려해  `포장 주문 상태`를 바꾼다.

### 7. 매장 주문

- `매장 주문`은 `매장 테이블을 점유`한 `손님`에게 조리된 음식을 제공한다.
- `매장 주문`은 현재 `매장 주문 순서`를 알 수 있는 `매장 주문 상태`를 가진다.
- `매장 주문 상태`은 이전 `매장 주문 상태`와 `매장 주문 순서`를 고려해 현재 `매장 주문 상태`를 바꾼다.
