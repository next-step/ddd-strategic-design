...
- `메뉴`는 주문 가능여부를 표현하는 `메뉴 게시`를 가진다.
- `메뉴`는 적합한 메뉴인지 확인하는 `메뉴 검증`을 가진다.
- `메뉴`는 `메뉴`의 카테고리 유형을 정의하는 `메뉴 카테고리`를 가진다.
- `메뉴`는 여러 `상품`들의 가격을 통해 이익을 검증하는 `마진`을 가진다.

#### 2. 주문

- `주문`은 여러 `메뉴`들을 가진다.
- `주문`은 `주문`의 방식을 선택할 수 있는 `주문 유형`을 가진다.
- `주문`은 현재 확인 가능한 주문의 진행상황을 보여주는 `주문 상태`를 가진다.
- `주문`은 현재 `주문`한 `메뉴`들을 확인할 수 있는 `주문 내역`을 가진다.
- `주문`은 `주문 내역`의 수량과 가격과 `주문`한 `메뉴`의 수량과 가격을 통해 유효한 `주문`인지 검증하는 `주문 검증`을 가진다.
- `주문`은 이전 `주문 상태`를 확인한다.

#### 3. 배달 주문

- `배달 주문`은 `배달 주문`의 흐름을 확인할 수 있는 `배달 순서`를 가진다.
- `배달 주문`은 `배달 기사`를 통해 배달을 진행한다.
- `배달 주문`은 `배달 기사`의 목적지인 `배달 주소`를 가진다.
- `배달 주문`은 이전 `배달 순서`를 확인한다.

#### 4. 포장 주문

- `포장 주문`은 `포장 주문`의 흐름을 확인할 수 있는 `포장 순서`를 가진다.
- `포장 주문`은 이전 `포장 순서`를 확인한다.

#### 5. 매장 주문

- `매장 주문`은 `매장 주문`의 흐름을 확인할 수 있는 `매장 순서`를 가진다.
- `매장 주문`은 `손님`이 `앉을 수` 있는 `매장 테이블`을 가진다.
- `매장 주문`은 이전 `매장 순서`를 확인한다.
