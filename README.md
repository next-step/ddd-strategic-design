...
| 메뉴 노출 | visible      | 메뉴를 노출한다. 노출할 메뉴의 가격은 음식을 개별적으로 주문하는 것보다 가격이 같거나 낮아야 한다.                                                                          |
...
| 매장 주문    | table service             | 매장에서 식사를 진행하는 주문 서비스                                                     |
| 테이블 지정   | select (table)            | 사용할 테이블을 선택한다. 매장 주문시 반드시 테이블이 지정되어야 한다.                                 |
| 테이블 정리   | clean (table)             | 사용한 테이블을 정리한다. 식사가 끝난 테이블을 다시 사용 가능한 상태로 변경한다.                           |
| 포장 주문    | take-out service          | 직접 매장에 방문하여 음식을 수령하는 주문 서비스                                              |
| 배달 주문    | delivery service          | 배달주소를 지정하여 배달 대행사를 통해 음식/상품을 수령하는 주문 서비스                                 |
