...
- 주문은 식별자(`id`), 주문 날짜(`orderDateTime`)를 가진다.
  - 주문 종류(`OrderType`)를 배달 주문(`DELIVERY`), 포장 주문(`TAKEOUT`), 매장 주문(`EAT_IN`) 3가지로 나뉜다.
  - 주문 상태(`OrderStatus`)를 주문 대기(`WAITING`), 주문 수락(`ACCEPTED`), 서빙(`SERVED`), 배달 중(`DELIVERING`), 배달 완료(`DELIVERED`), 주문 완료(`COMPLETED`) 6가지 중 하나로 가진다.
  - 1개 이상의 주문 항목(`OrderLineItem`)을 가진다.
    - 주문 수량(`quantity`)은 매장 주문일 경우 0 미만을 가질 수 있다. 
    - 주문 수량(`quantity`)은 포장 주문과 배달 주문일 경우 0 이상을 가진다.
    - 메뉴 가격과 일치하는 주문 가격(`price`)을 가진다. 
    - 등록된 메뉴(`Menu`)를 가진다.
      - 메뉴는 노출되어 있다.
  - 배달 주문은 비어있지 않은 배달 주소(`deliveryAddress`)를 가진다.
  - 매장 주문은 등록된 주문 테이블(`OrderTable`)을 가진다.
    - 주문 테이블은 비어있지 않다.
  
**기능**
- 주문을 등록한다. (``create``)
- 주문 대기 상태의 주문을 수락한다. (``accept``)
  - 배달 주문은 배달 대행사를 호출한다.
- 주문 수락 상태의 주문을 서빙한다. (``serve``)
