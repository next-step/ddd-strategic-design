...
- 메뉴(Menu)
  - 번호, 이름, 가격, 메뉴 그룹(MenuGroup), 메뉴 상품(Menu Product)으로 구성된다
    ...
    - 가격은 필수 값이며, 언제나 0보다 커야 한다.
      - 메뉴에 속한 상품 가격의 합보다 작거나 같아야 한다.
    - 메뉴 그룹에 속해야 한다
    - 메뉴 상품은 필수이다
  - 가격 변경이 가능하다
    - 언제나 0보다 커야 하며, 메뉴에 속한 상품 가격의 합보다 작거나 같아야 한다
  - 메뉴를 노출/미노출 할 수 있다
    - 노출할 때는 메뉴의 가격이 메뉴에 속한 상품 가격의 합보다 작거나 같아야한다
  - 등록된 메뉴를 조회 가능하다
- 메뉴 상품(Menu Product)
...
- 주문테이블(OrderTable)
  ...
  - 생성된 주문 테이블을 조회 가능하다
  - 주문 테이블의 착석하려는 손님의 숫자를 기록 할 수 있다
    - 손님의 숫자는 0보다 커야 하며, 이미 착석한 손님이 있는 테이블은 불가능하다
  - 주문 테이블에 손님이 착석할 수 있다 
    - 주문 테이블이 손님이 착석한 상태가 된다 
  - 주문 테이블을 치울 수 있다
    - 손님의 주문이 완료된 주문 테이블만 치우는 게 가능하다
  - 주문 테이블을 조회 가능하다 
- 배달 주문(Delivery Order)
  - 번호, 상태, 주문 메뉴(OrderLineItem), 배달 주소로 구성된다 
    - 번호는 자동 생성되는 값이며, 고유한 값이다
    - 주문 메뉴(OrderLineItem)는 필수 값이다
    - 비노출 메뉴는 주문이 불가능하다 
    - 배달 주소는 필수 값이다 
  - 주문의 상태 변경이 가능하다 
...
- 포장 주문(TakeOut Delivery)
  ...
  - 주문의 상태 변경이 가능하다
    - 대기 중인 상태를 수락 상태로 변경이 가능하다
    - 수락 상태의 주문을 제공 상태로 변경 가능하다 
    - 제공 상태의 주문을 완료 상태로 변경 가능하다
- 매장 주문(EatIn Delivery)
  - 번호, 상태, 주문 메뉴(OrderLineItem), 주문 테이블(OrderTable)로 구성된다
    - 번호는 자동 생성되는 값이며, 고유한 값이다
    - 주문 메뉴는 필수 값이다
      - 주문 메뉴의 개수는 0보다 커야 한다 
    - 비노출 메뉴는 주문이 불가능하다
  ...
  - 주문의 상태 변경이 가능하다
    ...
    - 수락 상태의 주문을 제공 상태로 변경 가능하다
    - 제공 상태의 주문을 완료 상태로 변경 가능하다
      - 완료 상태로 변경할 때 주문 테이블(OrderTable)을 비움처리 한다 
- 주문 메뉴(OrderLineItem)
  - 번호, 메뉴, 개수로 구성된다 
    - 번호는 자동 생성되는 값이며, 고유한 값이다 
    - 메뉴는 사전에 등록된 메뉴이다
