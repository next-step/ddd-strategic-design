...
| 메뉴제품 | Menu Product | 메뉴에 포함된 제품을 의미한다. |
...
| 주문상태 | Order Status | 주문의 상태를 의미한다. 조리중(COOKING), 식사중(EATING), 완료(COMPLETION)가 있다. |
| 조리중 | Cooking | 고객에게 주문을 받은 후의 주문상태로, 음식이 만들어지는 상태를 의미한다. |
| 식사중 | Eating | 고객에게 주문을 받은 후, 주문메뉴가 서빙되어 고객들이 식사 중일 때의 주문상태를 의미한다. |
