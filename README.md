...
| 주문을 등록     | createOrder      | 고객이 메뉴를 구매하기 위해 원하는 메뉴와 수량을 매장에 요청한 상태                                                                                              
| 접수 대기 중인 주문 | waitingOrder  | 고객의 주문이 매장 접수를 기다리고 있는 상태                                                                                                           |
| 접수된 주문     | acceptOrder      | 매장에서 고객의 주문 항목을 확인한 후, 주문 항목의 메뉴들이 판매 가능하면 주문을 접수한 상태                                                                               
| 제공된 주문     | servedOrder      | 고객에게 주문 내역에 있는 메뉴를 제공한 상태 |                                                                                                                            
| 배달중인 주문    | deliveringOrder  | 배달 주문을 한 고객에게 배달을 시작한 상황                                                                                                            |                                                                                                                                                                 
| 배달 완료된 주문  | deliveredOrder   | 배달 주문을 한 고객에게 배달을 완료한 상황                                                                                                            |                                                                                                                                                                  
| 완료된 주문     | completeOrder    | 주문의 모든 처리가 끝난 상황.             |
...
상품은 `상품 가격`과 `상품 명`을 가지고 있습니다.

### 4.메뉴
`메뉴`는 1개 이상의 `상품`을 묶어서 `메뉴상품`을 만들어 
`고객`에게 판매하는 단위입니다.<br/>
`메뉴`는 `노출된 메뉴`와 `숨겨진 메뉴`로 나뉩니다.<br/>
`메뉴`는 `메뉴 이름`과 `메뉴 가격`을 가지고 있습니다.<br/>
`메뉴 검증`은 `메뉴`의 정보와 `메뉴`를 구성하는 `메뉴 상품`들의 정보가 동일한지, 
`메뉴` 정보에 문제는 없는지 검증합니다.

### 5. 메뉴 상품
`메뉴 상품`은 한 `메뉴`의 `상품`, 그리고 그 `상품`의 `수량`을 포함하는 개념입니다.
`메뉴 상품 수량`은 `메뉴 상품`의 수량을 의미합니다.

### 6.메뉴 그룹
`메뉴 그룹`은 1개 이상의 `메뉴`를 묶어서 그룹으로 만든 것입니다.<br/>
메뉴 그룹은 `메뉴 그룹명`을 가지고 있습니다.
