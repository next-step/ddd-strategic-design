...
- 공백이거나 비속어를 포함되면 `name(이름)`이 될 수 없다.
- `price(가격)`은 0원 이상이어야 한다.
...
- 1개 이상의 `menuLineItems (메뉴 항목 목록)` 포함해야 한다.
- 공백이거나 비속어를 포함되면 `name(이름)`이 될 수 없다.
- `price (가격)`은 0원 이상이어야 하며 상품 목록 금액의 합은 메뉴의 가격보다 크거나 같아야 한다.
- `isDisplayed(노출 여부)`는 가격이 상품 목록 금액의 합보다 높을 경우 노출할 수 없다.
- `menuGroup (메뉴 그룹)` 반드시 특정 메뉴 그룹에 속해야 한다.
...
- 공백은 `name (이름)`이 될 수 없다.
- `numberOfGuests (방문한 손님 수)` 0명 이상이어야 한다.
- `empty (빈 테이블 여부)`
...
- 1개 이상의 `orderLineItems (주문 항목 목록)`을 포함해야 한다.
...
- `status (상태)` 는 `Waiting(접수 대기)`, `Accepted(접수)`, `Served(서빙됨)`, `Delivering(배달중)`, `Delivered(배달 완료)`, `Done(완료)` 가 있다.
- `orderTable (주문 테이블)`
  - `EatIn(매장)`일 때만 포함해야 한다.
- `deliveryAddress (배달 주소)`
  - `Delivery(배달)`만 공백일 수 없다.
