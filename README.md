...
| 가격   | price        | 상품의 가격. 0 또는 양수여야한다.                                        |
| 가격변경 | change price | 상품의 가격을 변경한다. 변경할 가격은 0 또는 양수여야한다.                          |
| 비속어  | purgomalum   | 음란한 표현, 욕설, 비속어.                                            |
...
| 한글명  | 영문명          | 설명                                                              |
|------|--------------|-----------------------------------------------------------------|
| 메뉴   | menu         | 손님에게 판매되기 위해 전시되는 정보. 1개 이상의 상품으로 이루어진다. (감자튀김 단품, 치즈버거 세트)     |
| -    | id           | 32글자로 이루어진 고유한 식별번호. (00000000-0000-0000-0000-000000000000)     |
| 이름   | name         | 메뉴의 이름. 공백이 아니어야 한다. 비속어를 포함할 수 없다.                             |
| 가격   | price        | 메뉴의 가격. 0 또는 양수여야한다. 메뉴에 속한 상품 금액의 합보다 작거나 같아야한다.               |
| 가격변경 | change price | 메뉴의 가격을 변경한다. 변경할 가격은 0 또는 양수여야하고, 메뉴에 속한 상품 금액의 합보다 작거나 같아야한다. |
| 전시   | display      | 손님에게 메뉴를 노출한다. 메뉴의 가격이 메뉴에 속한 상품 금액의 합보다 높을 경우 메뉴를 비전시한다.       |
| 비전시  | hide         | 손님에게 메뉴를 숨긴다.                                                   |
| 비속어  | purgomalum   | 음란한 표현, 욕설, 비속어.                                                |
...
| 한글명             | 영문명          | 설명                           |
|-----------------|--------------|------------------------------|
| 메뉴상품            | menu product | 메뉴와 상품간의 연관관계.               |
| -               | id           | 양의 정수로 이루어진 고유한 식별번호.        |
| 수량              | quantity     | 메뉴에 속한 상품의 수량                |
| 메뉴에 속한 상품 금액의 합 | -            | 메뉴에 속한 (상품의 금액 * 상품의 수량)들의 합 |
...
| 빈 테이블 여부 | occupied                | 주문테이블에 손님이 있는지 없는지 여부를 나타낸다. 빈 테이블에는 매장 주문을 등록할 수 없다.       |
| 빈 테이블 설정 | clear                   | 주문테이블을 빈 테이블로 설정한다. 완료되지 않은 주문이 있는 주문 테이블은 빈 테이블로 설정할 수 없다. |
| 빈 테이블 해지 | sit                     | 주문테이블을 비어있지 않은 테이블로 설정한다.                                   |

### 주문

| 한글명  | 영문명               | 설명                                                                                                 |
|------|-------------------|----------------------------------------------------------------------------------------------------|
| 주문   | order             | 손님이 메뉴와 수량 선택 후 식당에 요청한 형태                                                                         |
| -    | id                | 32글자로 이루어진 고유한 식별번호. (00000000-0000-0000-0000-000000000000)                                        |
...
| 서빙됨  | served            | 주문상태 중 하나. 완성된 상품을 손님이나 배달기사에게 서빙한 상태다.                                                            |
...
| 서빙   | serve             | 완료된 상품을 서빙한다. 기존 주문상태가 접수됨이 아닐 경우 서빙할 수 없다.                                                        |
