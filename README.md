...
| 접수       | accepted         | 주문이 접수되어 조리가 시작된 상태. <br/>주문 유형이 배달인 경우 키친라이더에 라이더 배정도 요청합니다.                                      |
| 서빙       | served           | 접수된 주문이 조리가 완료된 상태.                                                                                |
| 배달 중     | delivering       | 주문 유형이 배달일 때만 사용되는 상태로, <br/>조리가 완료된 주문이 라이더에게 전달되어 배달 주소로 배달 중인 상태                                |
| 배달 완료    | delivered        | 주문 유형이 배달일 때만 사용되는 상태로, <br/>라이더가 배달 주소로 배달을 완료한 상태.                                               |
| 완료       | completed        | 주문이 완료된 상태. <br/>주문 유형이 배달인 경우 배달 완료가 된 이후에 완료 처리가 가능합니다.<br/> 주문 유형이 매장식사인 경우 점유 중인 테이블이 없어야 합니다. |
...
- `MenuProduct`의 `price`는 `Product.price`와 `quantity`의 곱으로 계산된다.
...
- 관리자는 `Menu`를 생성할 수 있다.
    - 비속어 검증기로 `name`에 비속어 포함 여부를 확인해야 한다.
    - `Menu`의 `price`보다가 `menuProducts`에 `price`의 합계가 더 커야 한다.
- 관리자는 `Menu`의 `price`를 수정할 수 있다.
    - 변경되는 `Menu`의 `price`보다 `menuProducts`에 `price`의 합계가 더 커야 한다.
- 관리자는 `Menu`를 `display` 할 수 있다.
    - `Menu`의 `price`보다 `menuProducts`에 `price`의 합계가 더 커야 한다.
