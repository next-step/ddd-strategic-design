...
| 메뉴 전시   | MenuDisplay     | 메뉴가 손님에게 보이도록 한다.                                              |
| 메뉴 비전시  | MenuHide        | 메뉴가 손님에게 보이지 않도록 한다.                                           |
| 가게 테이블  | order_table     | 가게 테이블. 이름, 고객 수, 가득참 여부                                       |
| 주문      | orders          | 주문 정보(배달 주소, 주문 시각, 상태, 타입)                                    |
| 상품      | product         | 상품 정보(이름, 단일 가격)                                               |                                            |
| 주문 유형   | OrderType       | 주문의 유형은 3가지를 가진다. (배달주문, 방문포장, 완료)                             |
| 배달 주문   | Delivery        | 배달로 주문한다  (대기중 -> 접수 -> 서빙 -> 배달중 -> 배달완료 -> 완료)               |
| 방문 포장   | TakeOut         | 방문포장으로 주문한다  (대기중 -> 접수 -> 서빙 -> 완료)                           |
| 매장주문    | EatIn           | 매장 내부에서 주문한다  (대기중 -> 접수 -> 서빙 -> 완료 -> 테이블 정리)                |
