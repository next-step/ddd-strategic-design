...
public class InMemoryEatInOrderRepository implements EatInOrderRepository {
    ...
    @Override
    public boolean existsByRestraurantTableAndStatusNot(final RestaurantTable restaurantTable, final EatInOrderStatus status) {
        return orders.values()
                .stream()
                .anyMatch(order -> order.getOrderTable().equals(restaurantTable) && order.getStatus() != status);
    }
}
